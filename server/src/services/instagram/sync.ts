 ...
 *  Runs on a timer (IG_SYNC_INTERVAL_MS) and on demand from the UI.
 *
...
export async function syncInstagram(trigger = "manual"): Promise<SyncResult> {
  ...
  try {
    ...
    emitWorkspace("instagram:synced", {
...
export function startInstagramScheduler() {
  const interval = env.IG_SYNC_INTERVAL_MS;
  if (!isConfigured()) {
    console.log("  · instagram sync idle (META_ACCESS_TOKEN / IG_BUSINESS_ACCOUNT_ID not set)");
    return;
  }
  if (!interval || interval < 60_000) {
    console.log("  · instagram sync disabled (IG_SYNC_INTERVAL_MS)");
    return;
  }
  stopInstagramScheduler();

  // Run once shortly after boot so a restarted server doesn't wait hours
  // for the first snapshot of the day.
  setTimeout(() => void syncInstagram("startup").catch(() => undefined), 10_000);

  timer = setInterval(() => {
    void syncInstagram("scheduler").catch((e) => console.error("[instagram] scheduler error:", e?.message));
  }, interval);
  console.log(`  · instagram sync every ${Math.round(interval / 60_000)}m`);
}
