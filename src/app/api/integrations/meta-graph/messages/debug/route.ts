 ...
 * filter so it can be checked against production data directly. Removed
 * once the filter bug is found and fixed.
 */
export async function GET(req: Request) {
  ...
  const rows = await prisma.igConversation.findMany({
    ...
    take: 50,
    select: { id: true, igUsername: true, igName: true, lastMessageAt: true, lastMessageDirection: true, unread: true },
  });
